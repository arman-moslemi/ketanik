...
active!=0?
          setData(response.data.Data.filter(x=>x.BlogTypeID==active))
          :
  ...
  <DrawerPage drawers={drawers} name={"اخبار و مقالات"} navigation={navigation} />
    ...
    <View style={{display:'flex',flexDirection:'row-reverse',alignContent:'center',alignItems:'center'}}>
     ...
     <ScrollView horizontal={true} style={{display:'flex',flexDirection:'row-reverse'}}>
     {
                    type.map((item)=>{
                        return(
                          active==item.BlogTypeID?
                          <TouchableOpacity style={{ }}  onPress={()=>setActive(item.BlogTypeID)}>
                          <Text style={[styles.paragraph,{color:"#fff",backgroundColor:Colors.Yellow,borderRadius:50,paddingHorizontal:5}]}>
                          {item?.Name}      </Text> 
                            </TouchableOpacity>
                          :
      <TouchableOpacity onPress={()=>setActive(item.BlogTypeID)}>
